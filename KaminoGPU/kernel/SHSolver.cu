...
SHSolver::SHSolver(size_t nPhi, size_t nTheta, fReal radius, fReal frameDuration,
	...
	A(A), B(B), C(C), D(D), E(E)
{
	...
	this->velPhi = new SHQuantity("velPhi", nPhi, nTheta,
		vPhiPhiOffset, vPhiThetaOffset);
	this->velTheta = new SHQuantity("velTheta", nPhi, nTheta - 1,
		vThetaPhiOffset, vThetaThetaOffset);
	this->pressure = new SHQuantity("p", nPhi, nTheta,
		centeredPhiOffset, centeredThetaOffset);
	this->density = new SHQuantity("density", nPhi, nTheta,
		centeredPhiOffset, centeredThetaOffset);
	...
	int sigLenArr[1];
	sigLenArr[0] = nPhi;
	checkCudaErrors((cudaError_t)cufftPlanMany(&SHPlan, fftRank, sigLenArr,
		NULL, 1, nPhi,
		NULL, 1, nPhi,
		CUFFT_C2C, nTheta));
}
...
void SHSolver::copyVelocity2GPU()
{
	velPhi->copyToGPU();
	velTheta->copyToGPU();
}

void SHSolver::copyDensity2GPU()
{
	density->copyToGPU();
}
...
void SHSolver::determineLayout(dim3& gridLayout, dim3& blockLayout,
...
void SHSolver::precomputeABCCoef()
{
...
void SHSolver::stepForward(fReal timeStep)
{
...
void SHSolver::initDensityfromPic(std::string path)
{
	if (path == "")
	{
		return;
...
void SHSolver::initParticlesfromPic(std::string path, size_t parPerGrid)
{
	this->particles = new SHParticles(path, parPerGrid, gridLen, nTheta);
}

void SHSolver::write_data_bgeo(const std::string& s, const int frame)
{
...
void SHSolver::write_particles_bgeo(const std::string& s, const int frame)
{
	std::string file = s + std::to_string(frame) + ".bgeo";
	std::cout << "Writing to: " << file << std::endl;
...
void SHSolver::mapPToSphere(vec3& pos) const
{
	float theta = pos[1];
	float phi = pos[0];
...
void SHSolver::mapVToSphere(vec3& pos, vec3& vel) const
{
	float theta = pos[1];
	float phi = pos[0];
