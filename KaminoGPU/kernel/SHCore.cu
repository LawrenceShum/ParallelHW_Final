...
__device__ fReal SHLerp(fReal from, fReal to, fReal alpha)
{
...
__device__ fReal sampleVPhi(fReal* input, fReal phiRaw, fReal thetaRaw, size_t pitch)
{
		...
		|| thetaIndex == nThetaGlobal - 1)
	{
		...
		fReal lowerBelt = SHLerp(input[phiLower + pitch * thetaIndex],
			input[phiHigher + pitch * thetaIndex], alphaPhi);

		phiLower = (phiIndex + nPhiGlobal / 2) % nPhiGlobal;
		phiHigher = (phiLower + 1) % nPhiGlobal;

		fReal higherBelt = SHLerp(input[phiLower + pitch * thetaIndex],
			input[phiHigher + pitch * thetaIndex], alphaPhi);

		fReal lerped = SHLerp(lowerBelt, higherBelt, alphaTheta);
		return lerped;
	}
	else
	{
		...
		fReal lowerBelt = SHLerp(input[phiLower + pitch * thetaLower],
			input[phiHigher + pitch * thetaLower], alphaPhi);
		fReal higherBelt = SHLerp(input[phiLower + pitch * thetaHigher],
...
void SHSolver::projection()
{
	...
	checkCudaErrors((cudaError_t)cufftExecC2C(this->SHPlan,
...
SH::SH(fReal radius, size_t nTheta, fReal particleDensity,
...
void SH::run()
{
	SHSolver solver(nPhi, nTheta, radius, dt, A, B, C, D, E);
