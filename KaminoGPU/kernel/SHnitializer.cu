...
void SHSolver::initialize_velocity()
{
	std::cout << "Initializing velocity..." << std::endl;
	SHQuantity* u = this->velPhi;
	SHQuantity* v = this->velTheta;
...
fReal SHSolver::FBM(const fReal x, const fReal y) {
	fReal total = 0.0f;
...
fReal SHLerpHost(fReal from, fReal to, fReal alpha)
{
	return (1.0 - alpha) * from + alpha * to;
}

fReal SHSolver::interpNoise2D(const fReal x, const fReal y) const
{
	...
	fReal i1 = SHLerpHost(v1, v2, fractX);
	fReal i2 = SHLerpHost(v3, v4, fractX);
	return SHLerpHost(i1, i2, fractY);
}

fReal SHSolver::rand(const vec2 vecA) const
{
